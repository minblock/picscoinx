...
const char * const BITCOIN_CONF_FILENAME = "picscoinx.conf";
const char * const BITCOIN_PID_FILENAME = "picscoinxd.pid";
...
#else
    const char* pszModule = "picscoinx";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PicsCoinX";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/PicsCoinX";
#else
    // Unix
    return pathRet / ".picscoinx";
#endif
#endif
}
